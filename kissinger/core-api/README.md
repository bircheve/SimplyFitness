...
```bash
core-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
core-api$ sam local start-api
core-api$ curl http://localhost:3000/
```
...
```bash
core-api$ sam logs -n HelloWorldFunction --stack-name core-api --tail
```
...
```bash
sam delete --stack-name core-api
```
